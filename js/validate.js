...
function matchRoots(userX1: number | null, userX2: number | null, answerAnswer){
  if (answer.type === "none") return false;

  if (answer.type === "one") {
    const root = answer.x1 ?? 0;
    const values = [userX1, userX2].filter((v){ x1, x2 } = answer;
  if (x1 === undefined || x2 === undefined) return false;

  const values = [userX1, userX2].filter((v){
  if (user.mode === "none") return false;
  if (user.mode === "one") return user.x1.trim() === "";
  return user.x1.trim() === "" && user.x2.trim() === "";
}

export function validateAnswer(problem, user) {
  if (isEmpty(user)) {
...
export function formatAnswer(answerAnswer){
...
export function getDiscriminant(a: number, b: number, c: number){
  return b * b - 4 * a * c;
}
